...
async def run_tasks(config: ToxConfig, logger: logging.Logger) -> int:
    ...
    try:
        ...
        if config.run_parallel:
            ...
            for name in config.run_tasks:
                futures.append(loop.create_task(run_task(config.task(name), config.build,
            ...
            for name in config.run_tasks:
                if empty_line:
                    ...
                    empty_line = True
                results.append(await run_task(config.task(name), config.build, config.skip_missing_interpreters))
